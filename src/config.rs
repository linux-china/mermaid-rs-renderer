...
pub fn load_config_with_theme(
    ...
    if let Some(pie) = parsed.pie {
        apply_pie_config_file(&mut config.layout.pie, pie);
    }
...
/// Parse an aspect-ratio string like `16:9`, `4/3`, or `1.5`.
pub fn parse_aspect_ratio_value(raw: &str) -> Result<f32, String> {
    let value = raw.trim();
    if value.is_empty() {
...
fn apply_pie_config_file(target: &mut PieConfig, pie: PieConfigFile) {
    if let Some(v) = pie.render_mode {
        target.render_mode = v;
    }
    if let Some(v) = pie.use_max_width {
        target.use_max_width = v;
    }
    if let Some(v) = pie.text_position {
        target.text_position = v;
    }
...
/// Merge a parsed `%%{init: ...}%%` directive into a [`Config`].
///
/// This applies `theme`, `themeVariables`, `preferredAspectRatio`, and
/// per-diagram sections (`flowchart`, `pie`, `timeline`, `mindmap`, ...) on
/// top of the provided base config, mirroring how mermaid.js merges init
...
pub fn merge_init_config(mut config: Config, init: serde_json::Value) -> Config {
    ...
    if let Some(pie) = init.get("pie")
        && let Ok(pie_file) = serde_json::from_value::<PieConfigFile>(pie.clone())
    {
        apply_pie_config_file(&mut config.layout.pie, pie_file);
    }
